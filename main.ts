  ...
  do {
    console.log(
      chalk.magentaBright.bold.italic.underline(
        "\t\t WELCOME TO THE OBJECT-ORIENTED PROGRAMME OF SANDLEEN WASEEM"
      )
    );
    ...
    if (ans.select == "Sandleen Waseem") {
      console.log(
        chalk.redBright.bold(
          `\t\tYou're talking to ${chalk.bold.magentaBright("Sandleen Waseem")}`
        )
      );
    ...
    if (ans.select == "Student") {
      const answer = await inquirer.prompt({
        ...
        message: chalk.blueBright.bold(
          "Which student would you like to speak with? "
        ),
      });
      ...
      if (!student) {
        ...
        console.log(
          chalk.magentaBright.bold(`\t\tI am ${name.name}, and I am Good!! `)
        );
      ...
      if (student) {
        console.log(
          chalk.magentaBright.bold(
            `Hello, I am ${student.name} and I am Good!!....`
          )
        );
        console.log(persons.students);
      }
